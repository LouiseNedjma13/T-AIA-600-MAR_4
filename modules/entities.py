...
def extract_entities(text: str) -> dict[str, list[str]]:
    ...
    candidates = [
        _clean_entity(match.group())
        for match in ENTITY_PATTERN.finditer(normalized_text)
    ]
    candidates = [
        candidate for candidate in candidates if _is_entity_candidate(candidate)
    ]

    name_counter = Counter(candidates)
    location_counter = Counter(_extract_context_locations(normalized_text))
    _add_spacy_entities(normalized_text, name_counter, location_counter)
    ...
    return {"characters": characters, "locations": locations[:20]}
